...
export async function POST(request: NextRequest) {
  ...
  if (!title || !slug || !summary || !description || !locationCity || !locationProvince) return NextResponse.json({ error: 'Judul, ringkasan, deskripsi, dan lokasi wajib diisi.' }, { status: 400 })
  const status = ['published', 'closed'].includes(body.status) ? body.status : 'draft'
  try {
    const result = await execute(`INSERT INTO job_positions
      (title, slug, summary, description, responsibilities, qualifications, benefits, location_city, location_province, employment_type, work_mode, experience_level, education, salary_min, salary_max, application_deadline, published_at, status, created_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
